...
switch lower(input_data)
    case('real')
        ...
        im_in = imread('/Volumes/nick.antipa/MATLAB/Data/1deg_iccp/leaves_green.tif');
    ...
    case('simulation')
        if ~preload_matrices
             ...
             A_sub = in_inv.A_sub;
             in_for = load('/Users/nick.antipa/Documents/MATLAB/Output/1deg_20151129_A_sub_512_5_z260_gptie.mat');
             A_for = in_for.A_sub;
             if size(A_for,1)~-size(A_sub,2)
                 error('matrix sensor sizes do not match')
             end
        end
        lf_in = load('../Output/dragon_crop.mat');
        if monochrome
            b_raw = A_for*double(lf_in.lfb(:));
        end
        b = b_raw+randn(size(b_raw))*.005*prctile(b_raw,99);
        nsx = in_inv.settings.npx;    %Number of sensor pixels in x
        nsy = in_inv.settings.npy;  %Number of sensor pixels in y
        nx = in_inv.settings.N;   %Number of light field x bins
        ny = in_inv.settings.M;   %Number of light field y bins
        ntheta = in_inv.settings.P; %Number of theta bins
        
        nphi = in_inv.settings.Q; %Number of phi bins
...
elseif use_twist
    ...
    switch lower(model_type)
        ...
        case 'wavelet'
            ...
            dwvlt = 'db9';
            N = 4;
            [C,L] = wavedec2(zeros(nx,ny),N,wvlt);
            wvltsize = length(C);
            %             for m = 1:ntheta*nphi
...
for s = -1.4
    ...
    for m = 1:nphi
        for n = 1:ntheta
            ...
            im_shifted = interp2(IMX,IMY,sub_im,IMX+s*xshift+xc,IMY+s*yshift+yc);
